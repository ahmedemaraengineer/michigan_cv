...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train) :
    ...
    dists[i ,:] = ((X[i] - y)**2).sum(dim = 1).sqrt()
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = (torch.sum(X**2,dim=1).view(-1,1)+torch.sum(y**2,dim=1).view(1,-1)-2*torch.mm(X,y.t())).sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test) :
    ...
    mode_value ,mode_index = torch.mode(y_train[index])
    y_pred[i] = mode_value
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices :
    ...
    for i in range(num_folds) :
      ...
      fx_train = torch.cat([x_train_folds[j] for j in tmp])
      fy_train = torch.cat([y_train_folds[j] for j in tmp])
