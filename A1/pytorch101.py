...
def multiples_of_ten(start, stop):
  ...
  # Replace "pass" statement with your code
  a = []
  y = torch.arange(start ,stop+1 ,dtype = torch.float64)
  for i in(y) :
    if (i.item() % 10) == 0 :
      a.append(i.item())
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = x.reshape(2 ,3 ,4).transpose(1 ,2).reshape(8 ,3).t()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = torch.clone(x)
  args = torch.argmin(y ,dim = 1)
  y[torch.arange(y.shape[0]) ,args] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop == True : 
    for i in range(x.shape[0] - 1) :
      z = torch.stack((x[i].mm(y[i]) ,x[i+1].mm(y[i+1])) ,dim = 0)
  else :
    z = torch.bmm(x ,y)    
