...
def nn_forward_pass(params, X):
    ...
    neg_idxs = torch.where(hidden < 0)
    hidden[neg_idxs] = 0
    scores = hidden.mm(W2) + b2 # (N ,H) * (H ,C) = (N ,C)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # Replace "pass" statement with your code
    num_train = X.shape[0]
    maximum_scores = torch.max(scores ,1)[0].reshape(-1 ,1)
    scores -= maximum_scores
    out = torch.exp(scores) # (N , C)
    scores_sums=torch.sum(out, 1).reshape(N, 1)
    out /= scores_sums # Softmax scores    
    correct_class_idxs = (range(num_train) ,y)
    correct_scores = out[correct_class_idxs]
    ...
    # Replace "pass" statement with your code
    dl_dout = out.clone()
    dl_dout[range(num_train) ,y] -= 1
    dW2 = h1.t().mm(dl_dout) # (H ,N) * (N ,C) = (H ,C)
    dl_dh = dl_dout.mm(W2.t()) # (N ,C) * (C ,H) = (N ,H)
    dl_dh[h1 == 0] = 0 # the gradinet of the Relu function is implemented element-wise
    dW1 = X.t().mm(dl_dh) #  (D ,N) * (N ,H) * = (D ,H)
    db1 = torch.sum(dl_dh ,0)  # (H,)
    db2 = torch.sum(dl_dout ,0) # (C ,)
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  z1 = X.mm(params['W1']) + params['b1']
  h = torch.max(z1, torch.tensor([0.0]).cuda())
...
def nn_get_search_params():
  ...
  learning_rate_decays = [0.95 ,0.96 ,0.97]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  ...
  for i in learning_rates :
    for j in hidden_sizes :
      for k in regularization_strengths :
        ...
        net = TwoLayerNet(3 * 32 * 32, j, 10, device=data_dict['X_train'].device)
