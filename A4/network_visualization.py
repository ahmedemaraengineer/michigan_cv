...
def compute_saliency_maps(X, y, model):
  ...
  scores = model(X) # (N ,1000(classse))
  scores = scores.gather(1 ,y.view(-1 ,1)).squeeze()
  scores.backward(torch.ones(scores.shape[0] ,device = X.device))
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for _ in range(max_iter) :
    scores = model(X_adv) #(1 ,1000)
    _ ,y_pred = torch.max(scores ,1)
    if y_pred == target_y :
      break
    scores[0 ,target_y].backward()
    dX = X_adv.grad.data
    X_adv.data += learning_rate * dX/torch.norm(dX)
