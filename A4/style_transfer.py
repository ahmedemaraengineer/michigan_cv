...
def content_loss(content_weight, content_current, content_original):
    ...
    content_current = content_current.view(N ,C,-1)
    content_original = content_original.view(N ,C ,-1)
    loss = (torch.sum((content_original - content_current)**2)) * content_weight
    return loss
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.zeros([N ,C ,C]).to(features.device).to(features.dtype)
    for i in range(N) :
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i ,layer in enumerate(style_layers) :
      ...
      style_loss += (style_weight * torch.sum((gram_current - gram_source)**2))
