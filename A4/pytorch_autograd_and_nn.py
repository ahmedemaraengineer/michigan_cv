...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  conv_b1 = nn.init.zeros_(torch.empty(channel_1 ,device = device ,dtype = dtype))
  ...
  conv_b2 = nn.init.zeros_(torch.empty(channel_2 ,device = device ,dtype = dtype))
  conv_b2.requires_grad = True
  fc_w = nn.init.kaiming_normal_(torch.empty(num_classes ,channel_2*32*32 ,device = device ,dtype = dtype))
  fc_w.requires_grad = True
  fc_b = nn.init.zeros_(torch.empty(num_classes ,device = device ,dtype = dtype))
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    self.linear = nn.Linear(channel_2*32*32 ,num_classes )
...
def initialize_three_layer_conv_part4():
  ...
  ####################################################################################
  # Replace "pass" statement with your code
  model = nn.Sequential(OrderedDict([('conv1' ,nn.Conv2d(C ,channel_1 ,kernel_size = kernel_size_1 ,padding = 2)) ,
                                     ('relu1' ,nn.ReLU()) ,
                                     ('conv2' ,nn.Conv2d(channel_1 ,channel_2 ,kernel_size=kernel_size_2 ,padding = 1)) ,
                                     ('relu2' ,nn.ReLU()) ,
                                     ('flatten' ,Flatten()) ,
                                     ('linear' ,nn.Linear(channel_2*H*W ,num_classes))]))
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    blocks_list = [ResNetStem(Cin = Cin ,Cout = stage_args[0][0])]
    for i in range(len(stage_args)) :
      blocks_list.append(ResNetStage(*stage_args[i] ,block = block))
    self.cnn = nn.Sequential(*blocks_list)  
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
