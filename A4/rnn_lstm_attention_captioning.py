...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    dx = (dnext_h*(1 - (next_h**2))).mm(Wx.T)
    dprev_h = (dnext_h*(1 - (next_h**2))).mm(Wh.T)
    dWx = x.T.mm((dnext_h*(1 - (next_h**2))))
    dWh = prev_h.T.mm((dnext_h*(1 - (next_h**2))))
    db = (dnext_h*(1 - (next_h**2))).sum(axis = 0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T) :
      if i == 0 :
        ...
        h[: ,0 ,:] ,forward_cache = rnn_step_forward(x[: ,0 ,:] ,h0.clone() ,Wx ,Wh ,b)      

      else :

        h[: ,i ,:] ,forward_cache = rnn_step_forward(x[: ,i ,:], h[: ,i-1 ,:].clone(), Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for i in range(dh.shape[1]-1 ,-1 ,-1) :
      ...
      dx_step , dnext_h ,dWx_step ,dWh_step ,db_step = rnn_step_backward(dnext_h ,cache[i])

      if (i == dh.shape[1] - 1) :
        dx = torch.zeros([dx_step.shape[0] ,dh.shape[1] ,dx_step.shape[1]]).to(x1.dtype).to(x1.device)
        dh0 = torch.zeros([dx_step.shape[0] ,dh.shape[2]]).to(x1.dtype).to(x1.device)
        dWx = torch.zeros([dx_step.shape[1] ,dh.shape[2]]).to(x1.dtype).to(x1.device)
        dWh = torch.zeros([dh.shape[2] ,dh.shape[2]]).to(x1.dtype).to(x1.device)
        db = torch.zeros([dh.shape[2],]).to(x1.dtype).to(x1.device)
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        CNN_H0_Affine_W = torch.randn([hidden_dim ,input_dim] ,device = device ,dtype = dtype).div(math.sqrt(input_dim))
        ...
        H_Prob_Affine_W = torch.randn([vocab_size ,hidden_dim] ,device = device ,dtype = dtype).div(math.sqrt(hidden_dim))
        H_Prob_Affine_b = torch.randn([vocab_size] ,device = device ,dtype = dtype)
    ...
    def forward(self, images, captions):
        ...
        loss = temporal_softmax_loss(probs ,captions_out ,self._null)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T) :
      if i == 0 :
        h[: ,i ,:] ,c = lstm_step_forward(x[: ,i ,:] ,h0.clone() ,c0 ,Wx ,Wh ,b)
      else :

        h[: ,i ,:] ,c = lstm_step_forward(x[: ,i ,:] ,h[: ,i - 1 ,:].clone() ,c ,Wx ,Wh ,b)  
...
def dot_product_attention(prev_h, A):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    Mt = torch.matmul(prev_h.view(N ,1 ,H) ,A.view(N ,H ,16)).squeeze(1).div(H**0.5)
    attn_weights = F.softmax(Mt ,dim = 1)
    attn = torch.matmul(A.view(N ,H ,16) ,attn_weights.view(N ,16 ,1)).squeeze(2)
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    for i in range(T) :
      if i ==0 :
        ...
        h[: ,i ,:] ,c = lstm_step_forward(x[: ,i ,:] ,h0.clone() ,c0 ,Wx ,Wh ,b ,attn ,Wattn)
      else : 
        attn ,_ = dot_product_attention(h[: ,i - 1 ,:].clone() ,A)
        h[: ,i ,:] ,c = lstm_step_forward(x[: ,i ,:] ,h[: ,i-1 ,:].clone() ,c ,Wx ,Wh ,b ,attn ,Wattn)  
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
