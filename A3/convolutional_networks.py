...
class Conv(object):
      ...
      for j in range(F) :
        for k in range(H_prime) :
          for l in range(W_prime) :
            out[i ,j ,k ,l] = (x_pad[i ,: ,k * stride : (k * stride) + HH ,l * stride : (l * stride) + WW] * w[j ,: ,: ,:]).sum() + b[j]
  ...
  def backward(dout, cache):
    ...
    dw = torch.zeros(w.shape).to(w.dtype).to(w.device)
    db = torch.zeros(b.shape).to(b.dtype).to(b.device)
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    out = torch.zeros((N ,C ,H_dash ,W_dash)).to(x.dtype).to(x.device)
    for j in range(H_dash) :
      for k in range(W_dash) :
        out[: ,: ,j ,k] = torch.max(torch.max(x[: ,: ,j*stride : j*stride + pool_height ,k*stride : k*stride + pool_width] , -1)[0] ,-1)[0]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
  ...
  def backward(dout, cache):
    ...
    H_dash = 1 + (H - pool_height) // stride
    W_dash = 1 + (W - pool_width) // stride
    dx = torch.zeros((N ,C ,H ,W)).to(x.dtype).to(x.device)

    for i in range(N) :
      for j in range(C) :
        for k in range(H_dash) :
          for l in range(W_dash) :
            ind = torch.argmax(x[i ,j ,k*stride : k*stride + pool_height ,l*stride : l*stride + pool_width])
            ind1 = ind // pool_height
            ind2 = ind % pool_width
            dx[i ,j ,k*stride : k*stride + pool_height ,l*stride : l*stride + pool_width][ind1 ,ind2] = dout[i ,j ,k ,l]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    out_3 ,cache_3 = Linear.forward(out_2 ,W3 ,b3)
    scores = out_3
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    loss += self.reg*torch.sum(torch.tensor([torch.sum(self.params['W' + str(i)]**2) for i in [1 ,2 ,3]]))
    dout ,grads['W3'] ,grads['b3'] = Linear.backward(dout ,cache_3)
    grads['W3'] += self.reg * 2 * self.params['W3']
    dout ,grads['W2'] ,grads['b2'] = Linear_ReLU.backward(dout ,cache_2)
    grads['W2'] += 2 * self.reg * self.params['W2']
    _ ,grads['W1'] ,grads['b1'] = Conv_ReLU_Pool.backward(dout ,cache_1)
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i in range(len(num_filters)) :
      if i == 0 :
        if weight_scale == 'kaiming' :
          self.params['W' + str(i + 1)] = kaiming_initializer(layers_dims[i] ,C ,K = 3 ,relu = True ,device = device ,dtype = dtype)
        else :
        ...
        if weight_scale == 'kaiming' :
          self.params['W' + str(i + 1)] = kaiming_initializer(layers_dims[i] ,layers_dims[i - 1] ,K = 3 ,dtype = dtype ,device = device) 
        else :
    ...
    if weight_scale == 'kaiming' :
      self.params['W' + str(i + 1)] = kaiming_initializer(num_filters[i - 1] * H * W // down_sample**2 ,layers_dims[i] ,K = None,dtype = dtype ,device = device) 
    else :
      self.params['W' + str(i + 1)] = weight_scale * torch.randn(num_filters[i - 1]*H*W // down_sample**2 ,layers_dims[i]).to(dtype).to(device)
    self.params['b' + str(i + 1)] = torch.zeros((layers_dims[i])).to(dtype).to(device)  
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1) :
      scores ,cache = FastConv.forward(scores ,self.params['W' + str(i + 1)] ,self.params['b' + str(i + 1)] ,conv_param)
      L2reg += torch.sum(self.params['W' + str(i + 1)] ** 2)
      cache_history.append(cache)

      if self.batchnorm :
        scores ,cache = SpatialBatchNorm.forward(scores ,self.params['gamma' + str(i + 1)] ,self.params['beta' + str(i + 1)] ,self.bn_params[i])
        cache_history.append(cache)
    ...
    L2reg += torch.sum(self.params['W' + str(self.num_layers)] ** 2)
    L2reg *= reg
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dout ,grads['W' + str(self.num_layers)] ,grads['b' + str(self.num_layers)] = Linear.backward(dout ,cache_history.pop())
    grads['W' + str(self.num_layers)] += 2 * reg * self.params['W' + str(self.num_layers)]
    while i >= 0 :
      if i in self.max_pools :
        dout = FastMaxPool.backward(dout ,cache_history.pop())
      dout = ReLU.backward(dout ,cache_history.pop())
      if self.batchnorm :
        dout ,grads['gamma' + str(i + 1)] ,grads['beta' + str(i + 1)] = SpatialBatchNorm.backward(dout ,cache_history.pop())
      dout ,grads['W' + str(i + 1)] ,grads['b' + str(i + 1)] = FastConv.backward(dout ,cache_history.pop())
      grads['W' + str(i + 1)] += 2 * reg * self.params['W' + str(i + 1)]
      i -= 1  
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def find_overfit_parameters():
  weight_scale = None  # Experiment with this!
  learning_rate = None  # Experiment with this!
  ############################################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims = input_dims ,num_classes = 10 ,
                      num_filters = [32 ,32 ,64 ,128 ,128] ,
                      max_pools = [1 ,3 ,4] ,
                      weight_scale = 'kaiming' ,
                      batchnorm = False ,
                      reg = 1e-5 ,device = device)
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      x_norm = (x - mean) / ((var + eps)**(1/2))
      out = (gamma * x_norm) + beta
      running_mean = (momentum * running_mean) + ((1 - momentum) * mean)
      running_var = (momentum * running_var) + ((1 - momentum) * var)
      cache = (x_norm ,mean ,var ,eps ,gamma ,beta ,x)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
      ...
      #######################################################################
      # Replace "pass" statement with your code
      x_norm = (x - running_mean) / ((running_var + eps)**(1/2))
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x_norm ,mean ,var ,eps ,gamma ,beta ,x = cache
    N ,D = x.shape
    # out = (x_norm * gamma) + beta
    dgamma = torch.sum(dout * x_norm ,0)
    dbeta = torch.sum(dout ,0)
    # out = ((x - mean) / (var + eps)**(1/2)) * gamma
    dx_norm = dout * gamma # (1)
    # x_norm = (x - mean) / ((var + eps)**(1/2))
    dx_mean1 = dx_norm * (1 / ((var + eps)**(1 /2)))
    divar = torch.sum(dx_norm * (x - mean), 0)
    dvar = divar * -1 / 2 * (var + eps) ** (-3/2)
    dsq = 1 / N * torch.ones((N, D)).to(x.dtype).to(x.device) * dvar
    dx_mean2 = 2 * (x - mean) * dsq
    dx1 = dx_mean1 + dx_mean2
    dmean = -1 * torch.sum(dx_mean1 + dx_mean2, 0)
    dx2 = 1 / N * torch.ones((N, D)).to(x.dtype).to(x.device)* dmean
    dx = dx1 + dx2    
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward_alt(dout, cache):
    ...
    dgamma = torch.sum(dout * x_norm ,0)
    dbeta = torch.sum(dout ,0)
    dx = (dout * gamma - (dout * gamma).sum(0)/N - (dout * gamma * x_norm).sum(0) * x_norm/N)/var**(1/2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
