...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    x_train = x.reshape(x.shape[0] ,-1) # (N ,D)
    out = x_train.mm(w) + b # (N ,M)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b)
  ...
  def backward(dout, cache):
    ...
    dx = dout.mm(w.t()).reshape(x.shape) # (N ,M) * (M ,D) = (N ,D)
    dw = x_train.t().mm(dout) # (D ,N) * (N ,M) = (D ,M)
    db = dout.sum(dim = 0) # (N ,M)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class ReLU(object):
  ...
  def backward(dout, cache):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    dx = dout * torch.tensor(x.clone().detach() > 0 ,dtype = dout.dtype ,device = dout.device)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dx2 ,dW2 ,db2 = Linear.backward(soft_grad ,cache2)
    _ ,dW ,db = Linear_ReLU.backward(dx2 ,cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers) :
      self.params['W' + str(i + 1)] = weight_scale * torch.randn((dims[i] ,dims[i + 1])).to(dtype).to(device)
      self.params['b' + str(i + 1)] = torch.zeros(dims[i + 1]).to(dtype).to(device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1) :
      scores ,cache['cache' + str(i + 1)] = Linear_ReLU.forward(scores ,self.params['W' + str(i + 1)] ,self.params['b' + str(i + 1)])
      L2reg += torch.sum(self.params['W' + str(i + 1)] ** 2)
      if self.use_dropout :
        scores ,cache['drop' + str(i + 1)] = Dropout.forward(scores ,self.dropout_param)
    scores ,cache['cache' + str(self.num_layers)] = Linear.forward(scores ,self.params['W' + str(self.num_layers)] ,self.params['b' + str(self.num_layers)])
    ...
    loss += L2reg
    dout ,grads['W' + str(self.num_layers)] ,grads['b' + str(self.num_layers)] = Linear.backward(dout ,cache['cache' + str(self.num_layers)])
    grads['W' + str(self.num_layers)] += 2.0 * self.reg * self.params['W' + str(self.num_layers)]

    for i in range(self.num_layers - 1) :
      if self.use_dropout :
        dout = Dropout.backward(dout ,cache['drop' + str(self.num_layers - (i + 1))])
      dout ,grads['W' + str(self.num_layers - (i + 1))] ,grads['b' + str(self.num_layers - (i + 1))] = Linear_ReLU.backward(dout ,cache['cache' +str(self.num_layers - (i +1))])
      grads['W' + str(self.num_layers - (i + 1))] += 2.0 * self.reg * self.params['W' + str(self.num_layers - (i + 1))]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model , data_dict ,device = 'cuda' ,optim_config = {'learning_rate' :1e-1} ,lr_decay = 0.94 ,num_epochs = 10 ,update_rule=sgd)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - config['learning_rate'] * dw/(config['cache']**(1/2)+config['epsilon'])
  ###########################################################################
  #                             END OF YOUR CODE                            #
  ###########################################################################
...
def adam(w, dw, config=None):
  ...
  next_w = w - config['learning_rate'] * tmp_m/(config['epsilon'] + tmp_v ** (1/2))
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
